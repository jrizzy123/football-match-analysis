...
def main():
    video_frames = read_video('input_videos/chelseapsg.mp4')
    ...
    for frame_num, player_tracks in enumerate(tracks['players']):
        ...
        if assigned_player != -1:
            ...
            team_ball_controls.append(tracks['players'][frame_num][assigned_player]['team'])
        else:
            team_ball_controls.append(team_ball_controls[-1])
    team_ball_control = np.array(team_ball_controls)
    ...
    save_video(output_video_frames, 'output_videos/chelseapsg_output.mp4')
